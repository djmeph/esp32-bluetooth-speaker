...
This is a simple design for 3D printing and building your own rechargeable Bluetooth speaker based on the ESP32-Audio-Kit developer board. The firmware we will use is Squeezelite, which is much more than a Bluetooth audio platform. With Squeezelite you can also connect to WiFi and play streaming music through Apple AirPlay, Spotify Connect, and Home Assistant Media Player. You can also sync multiple devices together to play music through your entire house, and there are hardware upgrades available that won't be covered in this tutorial. You will also find step files in this repo to remix and modify the device for your own needs. 

## Credits

This work is based on a few open source projects and design contributions from throughout the maker community. 

- [esp32-squeezelite]([https://github.com/sle118/squeezelite-esp32?tab=readme-ov-file]) - The ESP32 firmware that makes this all possible! 
- [EdwardKrayer's ESP32-A1S_Squeezebox_Case](https://github.com/EdwardKrayer/ESP32-A1S_Squeezebox_Case) - The 3D printer designs contained in this repo were built off of this design. 
...
- 2X 18650 Lithium-Ion button top batteries ([Preferably 9800-10000mAh](https://www.google.com/search?q=18650+9900mah+battery))
- 3D Printer filament of your choice, PLA or PETG recommended
- 2X [2" 4Ohm 5W Full Range Speakers](https://www.amazon.com/DWEII-Speaker-Loudspeaker-Compatible-Arduino/dp/B0CH89VKK3?th=1)
- 3X [JST XH 2.54mm pitch 2-pin electronic computer connector IC male plugs](https://www.jst.com/products/crimp-style-connectors-wire-to-board-type/xh-connector/)
- ~80cm 22ga stranded wire
...
- 12X M3x6mm hex socket head cap screws
- 8X M3x8mm hex socket head cap screws
- 4X M3x10mm hex socket head cap screws
- 8X M3 hex nuts
- 8X M3 hex washers
...
- needle nose pliers
- [JST crimping tool](https://www.adafruit.com/product/350?srsltid=AfmBOoptPi8FYWA-7s8cKBnXNS4IoNQylDs4XngL9c6RnNxk61b_cTp7) (or buy three pairs of wires pre-crimped)
...
I have been mostly printing with PLA, with a preference for matte carbon fiber PLA for the main body components.
...
- First install the battery shield into the battery crade of the body. Make sure to line up the button and the USB-C port with the windows on the back of the body before screwing it in. **IMPORTANT** Do not tighten the screws too tight. Ideally, they should not be torqued at all, just lightly fastened to the inserts until they touch the circuit board. This has caused me a bit of grief, because over-tightening the battery shield will cause it to smoke when the batteries are inserted, and burns out one of the microchips on the bottom of the board. This bricks the battery shield. It's a nice device otherwise. Once it has been lightly fastened it will not need to be adjusted or interfered with again. 
- Next, attach the speakers and grills to the speaker panel. The grills should go on top, with four M3x8mm screws each, through the panel, and into the speakers. Put a washer and nut on each screw and use a pair of needle-nose pliers to hold the nut while tightening the screws. Using M3 lock washers is also perfectly fine. Secure these screws tight to the panel and ensure there is no loose attachments that can vibrate. There should be enough room in the speaker cavities to get your fingers inside to attach the washers and nuts to the screws from behind. 
- Next, inser the ESP32-Audio-Kit board onto the pegs in the top cavity of the speaker panel. In my case, there wasn't much tolerance, so I had to line up all four pegs with the holes in the circuit board before pressing down.
- Guide the wires through the sides of the top cavity and plug in the JST connectors for both speakers, and the 5V power source. These are all labeled on the board, but here is a reference in case you have trouble reading it:
- Ensure that the switch on the front of the battery shield is in the `on` position and press the button on the back of the body once. If everything is connected correctly, you should be able to connect to it as a bluetooth device and start testing the sound. Make sure both speakers are working before advancing to the final steps. Once you are satisfied that everything is working correctly, press the same button on the back twice to turn the power off. 
...
- Finally, use the four M3x10mm to secure the cover to the panel. Do not force or push the screws into these holes, they should self-tap. 
...
- Press the power button the back once to power on the device. 
